...
`face_swap.py` adds a real-time identity swap that keeps the actual webcam frame: SCRFD detection (`buffalo_l/det_10g`), ArcFace identity (`w600k_r50`), `inswapper_128` or `hyperswap_1a_256`, optional GPEN-BFR-256 / GFPGAN 1.4 enhancement, a blurred box-mask paste-back and the existing latest-frame Spout transport (`ComfyLiveFaceSwap`). Every model runs as an ONNX Runtime **DirectML** session (`dml_device_id` 1 = R9700 on this host, 0 = RX 9070 XT); the ROCm torch devices are not involved and the `insightface` package is not imported (its `onnx` import collides with the protobuf pin of the installed audio packs). Models are not downloaded automatically; expected files, sizes, SHA-256 values and licences are listed in `tools/upgrade_v099.py` and `docs/LIVE_FACE_SWAP_V099.md`.

Nodes: **Face Swap Models · DirectML** (cached sessions), **Face Swap Identity from Images** (averaged ArcFace embedding of the largest face per image), **Face Swap Image Preview** (offline batch swap with per-stage milliseconds) and **Live Face Swap Webcam → Spout** (blocking; stop with Interrupt, never Run (Instant)). `CaptureWorker` gained an `output_size` argument so the live node receives full-resolution frames while the LivePortrait paths keep their 256 px input.

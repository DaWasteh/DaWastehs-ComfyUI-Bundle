#!/bin/bash
# Post-crash continuation: fresh server per area, memory guard between runs.
source "l:/GitHub/DaWastehs-ComfyUI-Bundle/performance/rdna4/bench/lib.sh"
start_server v098_baseline
run --prompt api/TRAIN_ZImage_bench.json --test-id TRAIN-ZI --variant v098_baseline --run-type cold --seed 42 --workload "$W_TR" $R --out raw/TRAIN-ZI --timeout 2400
guard; run --prompt api/TRAIN_ZImage_bench.json --test-id TRAIN-ZI --variant v098_baseline --run-type warm --seed 43 --set "8.seed=43" --workload "$W_TR" $R --out raw/TRAIN-ZI --timeout 2400
start_server v098_baseline

# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.3
...
- **4.556 automatisch zugeordnete Parameter-Notes** erklären jeden Nicht-Pixaroma-/Nicht-Dokumentations-Node einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
Automatisch geprüft werden alle 227 Workflows. Vor dem Release rekonstruiert `--against-head` die v0.9.3-AutoSongwriter-Konsolidierung deterministisch aus dem noch auf v0.9.2 stehenden `HEAD`; nach dem Release bewahrt `--baseline-ref v0.9.2` denselben Vergleich. Der Validator prüft zusätzlich die exakte Dateimenge und akzeptiert die zwei neuen Genre-Selector-Workflows nur, wenn sie byte-/semantikgleich aus ihren jeweiligen v0.9.2-POP-Quellen erzeugt werden:
...
- 10.028 Nodes und 4.556 Dokumentations-/Parameter-Notes; die verbleibenden `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.927 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen, AutoSongwriter-Genre/BPM/Tonart/Sprache/Dateiname-Verbindungen, Sekunden→Frame-Umrechnungen und wechselseitiger Input-/Output-Zuordnung erfasst

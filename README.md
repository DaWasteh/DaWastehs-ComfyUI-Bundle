# DaWasteh – konsolidierte ComfyUI-Workflows · v0.8.6
...
- **256 kuratierte Workflow-Dateien** in 31 Kategorien
- **21 zusätzliche Dual-GPU-Workflows**, je einer pro unterstützter Modellfamilie, für einen gemeinsamen ComfyUI-Server auf Port 8188: Diffusionsmodelle auf der R9700 (`gpu:0`), CLIP/Textencoder und VAEs auf der RX 9070 XT (`gpu:1`)
- **alle 9 MiniMax-H3-Workflows** verwenden die empfohlene pruned-ComfyUI-Turbo-LoRA `v4 step-600 EMA` mit dem qualitätsorientierten Profil aus 8 Schritten, Euler, Beta, Video-Sigma 12 und Audio-Sigma 4; FL2VA und Ref2VA wurden auf beiden GPUs live bis zum dekodierten 5-Frame-Ergebnis geprüft
...
- 239 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
- alle vorhandenen Workflows wurden mit deutlich größeren, nachvollziehbaren Abständen angeordnet
- **3.811 automatisch zugeordnete Parameter-Notes** erklären jeden Nicht-Pixaroma-/Nicht-Dokumentations-Node einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
- 9 lokale MiniMax-H3-Workflows: FL2VA, Ref2VA, automatische Audio-/Videolänge sowie ein Complete-Song-Director, der lange Songs seriell in höchstens 15 Sekunden lange H3-Szenen zerlegt. v0.8.4 ergänzte Identity-Lock und unverändertes Originalaudio; v0.8.6 ergänzt die geprüfte 8-Step-Turbo-LoRA in jedem sichtbaren und intern erzeugten H3-Graphen sowie zwei Guide-basierte Prompt-Enhancer. Einrichtung und Prüfnachweise: [docs/MINIMAX_H3_TURBO_AND_PROMPTS.md](docs/MINIMAX_H3_TURBO_AND_PROMPTS.md)
...
| Ordner | Anzahl | Zweck |
|---|---:|---|
| `Dual GPU - R9700 + RX 9070 XT` | 21 | Je ein kuratierter Port-8188-Workflow pro offiziell unterstützter Modellfamilie; vollständige Einrichtung: [docs/DUAL_GPU_WORKFLOWS.md](docs/DUAL_GPU_WORKFLOWS.md) |
...
Automatisch geprüft wurden alle 256 Workflows. Der `--against-head`-Modus prüft neue und geänderte Dateien vollständig, behandelt bereits eingecheckte Integrationen als unveränderte Baseline, normalisiert bei älteren Deltas ausschließlich die im 186-Dateien-Manifest erlaubten Prompt-/Pause-Transformationen und akzeptiert die H3-Turbo-Migration nur dann, wenn sie bytegenau aus dem deterministischen Integrator reproduzierbar ist:

- 256/256 gültige JSON-Dateien
- 239 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 307 Haupt- und Untergraphen rekursiv geprüft
- 9.100 Nodes und 3.811 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.261 Graph-Links einschließlich Slotgrenzen, Typkompatibilität und wechselseitiger Input-/Output-Zuordnung erfasst; die Workflow-12-Metrikpfade und alle acht Workflow-13-Ausgaben sind strukturell verbunden

# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.5
...
- **v0.9.5: Godot-Startworkflows und echter Live-Voice-Swap-Control-Pfad**: Zwei neue Workflows unter `Game Development` machen aus den fehlerhaften Gemini-Skizzen belastbare lokale Startpunkte. FLUX.2 Klein 4B erzeugt ein 1024er PS1-Materialkonzept, das Core-Nodes auf echte 128×128 Pixel und 32 Farben reduzieren; Hunyuan3D 2.1 nutzt den bewährten Sampling-Shift und ComfyUI-0.34-Core-`DecimateMesh` für ein statisches Low-Poly-GLB. Beide verwenden ausschließlich bereits vorhandene Modellgewichte und dokumentieren ehrlich, dass weder ein Konzeptblatt ein UV-Atlas noch das GLB automatisch texturiert/geriggt ist. Der separate `RVC_DirectML-Live-Microphone-Voice-Swap`-Workflow startet, identifiziert und stoppt ausschließlich den vollständig hash-verifizierten b2332-Runtime. OmniVoice wurde anhand offizieller Quellen verworfen: schnelle Zero-shot-TTS ja, aber kein echtes Live-Speech-to-Speech-Streaming; die offiziellen Gewichte sind zudem CC-BY-NC. Details: [Godot-Guide](docs/GODOT_GAME_ASSET_START.md), [Live-Voice-Guide](docs/LIVE_VOICE_SWAP_V095.md), [OmniVoice-Prüfung](docs/OMNIVOICE_LIVE_SWAP_EVALUATION.md)
...
- **4.596 Dokumentations-/Parameter-Notes** erklären die Workflow-Nodes einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
| `Game Development` | 2 | FLUX.2-Klein-PS1-Texturkonzept mit echter Pixel-/Palettenstufe sowie Hunyuan3D-Low-Poly-Static-GLB für Godot |
...
Automatisch geprüft werden alle 230 Workflows. Vor dem Release rekonstruiert `--against-head` die v0.9.5-Ergänzungen deterministisch aus dem noch auf v0.9.4 stehenden `HEAD`; nach dem Release bewahrt `--baseline-ref v0.9.4` denselben Vergleich. Der Validator prüft zusätzlich die exakte Dateimenge, die früheren AutoSongwriter-/Wan-Animate-Upgrades und die drei reproduzierbaren v0.9.5-Ableitungen:
...
- 10.117 Nodes und 4.596 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.976 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen, AutoSongwriter-Genre/BPM/Tonart/Sprache/Dateiname-Verbindungen, Sekunden→Frame-Umrechnungen und wechselseitiger Input-/Output-Zuordnung erfasst
...
- Hunyuan3D 2.1 lief mit neuem Seed über den gepflegten Shift-1-/40-Schritt-Pfad und den Core-`DecimateMesh` in 65,1 Sekunden bis zu einem gültigen 72.168-Byte-GLB mit einem Mesh, 2.012 Vertices und 3.728 Faces; wie dokumentiert ohne Bildtexturen, Skin oder Animationen

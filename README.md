# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.0
...
- **262 kuratierte Workflow-Dateien** in 31 Kategorien
- **27 zusätzliche Dual-GPU-Workflows** für einen gemeinsamen ComfyUI-Server auf Port 8188: Jeder besitzt genau einen zentralen `DaW Multi-GPU Device Control`-Node mit getrennten MODEL-, CLIP- und VAE-Dropdowns; voreingestellt sind R9700 (`gpu:0`) für Diffusion sowie RX 9070 XT (`gpu:1`) für CLIP/Textencoder und VAEs
...
- 245 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
...
| Ordner | Anzahl | Zweck |
|---|---:|---|
| `Dual GPU - R9700 + RX 9070 XT` | 27 | 23 kuratierte Modellfamilien-Workflows, offene MiniMax-H3-FL2VA-/Ref2VA-Varianten sowie LTX-2.5 T2V/I2V/FLF2V und Wan Animate 2 Motion Transfer für Port 8188; Einrichtung: [docs/DUAL_GPU_WORKFLOWS.md](docs/DUAL_GPU_WORKFLOWS.md) |
...
| `Prompt Enhancer` | 13 | Gemma/Qwen Prompt-, Bild-, Audio- und Videoverständnis sowie getrennte, auf den bereitgestellten MiniMax-Regeln basierende H3-Base-/Ref2VA-Enhancer |
| `Music Generation` | 30 | ACE-Step, Stable Audio 3, YuE CoT/ICL, HeartMuLa, drei INT8-Varianten sowie Qwen-/Gemma-Idee-zu-Lyrics und AutoSongwriter-Presets |
...
Automatisch geprüft wurden alle 262 Workflows. Der `--against-head`-Modus prüft neue und geänderte Dateien vollständig, behandelt bereits eingecheckte Integrationen als unveränderte Baseline, normalisiert bei älteren Deltas ausschließlich die im 186-Dateien-Manifest erlaubten Prompt-/Pause-Transformationen und akzeptiert die H3-Turbo-Migration nur dann, wenn sie bytegenau aus dem deterministischen Integrator reproduzierbar ist:

- 262/262 gültige JSON-Dateien
- 245 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 319 Haupt- und Untergraphen rekursiv geprüft
- 9.806 Nodes und 4.148 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.925 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen und wechselseitiger Input-/Output-Zuordnung erfasst; die Workflow-12-Metrikpfade und alle acht Workflow-13-Ausgaben sind strukturell verbunden
- keine neuen doppelten IDs, fehlenden oder einseitigen Endpunkte, Note-Zuordnungs- oder Layoutfehler in den v0.9.0-Dateien

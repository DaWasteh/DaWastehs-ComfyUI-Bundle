# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.1
...
- **265 kuratierte Workflow-Dateien** in 31 Kategorien
- **28 zusätzliche Dual-GPU-Workflows** für einen gemeinsamen ComfyUI-Server auf Port 8188: Jeder besitzt genau einen zentralen `DaW Multi-GPU Device Control`-Node mit getrennten MODEL-, CLIP- und VAE-Dropdowns; standardmäßig liegt Diffusion auf der R9700 (`gpu:0`) und CLIP/VAE auf der RX 9070 XT (`gpu:1`). MiniMax Music 3 kehrt diese MODEL-/CLIP-Verteilung wegen des großen BF16-Autoregressionsstacks bewusst um.
- **v0.9.1: MiniMax Music 3 lokal und live geprüft**: ein offizielles Template-basiertes FP32-DiT/BF16-Text-to-Music-Workflow, eine passende Dual-GPU-Variante und ein auf MiniMax' offiziellem Caption-Rewriter-Skill basierender Prompt-Enhancer. Normaler und Dual-GPU-Graph wurden mit vier Sekunden Maximaldauer, allen 30 Euler-/Simple-Schritten und tiled DAV decode bis zu nichtleeren 7,988-Sekunden-Stereo-FLACs ausgeführt; der Enhancer lieferte live exakt seine drei Pflichtabschnitte. Ein LoRA-Trainingsworkflow wird bewusst nicht als funktionsfähig ausgeliefert, weil MiniMax keine Trainingsrecipe veröffentlicht hat und ComfyUIs Music-3-DAV decoder-only ist. Details: [docs/MINIMAX_MUSIC3.md](docs/MINIMAX_MUSIC3.md)
...
- 248 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
- alle vorhandenen Workflows wurden mit deutlich größeren, nachvollziehbaren Abständen angeordnet
- **3.902 automatisch zugeordnete Parameter-Notes** erklären jeden Nicht-Pixaroma-/Nicht-Dokumentations-Node einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
- 9 lokale MiniMax-H3-Quellworkflows: FL2VA, Ref2VA, automatische Audio-/Videolänge sowie ein Complete-Song-Director, der lange Songs seriell in höchstens 15 Sekunden lange H3-Szenen zerlegt. v0.8.4 ergänzte Identity-Lock und unverändertes Originalaudio; v0.8.6 ergänzte Turbo-LoRA und Prompt-Enhancer; v0.8.7 liefert zusätzlich zwei offene Dual-GPU-Arbeitsgraphen für normale FL2VA- und Ref2VA-Anwendungen außerhalb des Musik-Directors; v0.8.8 verbindet alle sichtbaren und internen Selector-Pfade mit einer zentralen manuellen GPU-Steuerung. Einrichtung und Prüfnachweise: [docs/MINIMAX_H3_TURBO_AND_PROMPTS.md](docs/MINIMAX_H3_TURBO_AND_PROMPTS.md)
...
| Ordner | Anzahl | Zweck |
|---|---:|---|
| `Dual GPU - R9700 + RX 9070 XT` | 28 | 24 kuratierte Modellfamilien-Workflows einschließlich MiniMax Music 3, offene MiniMax-H3-FL2VA-/Ref2VA-Varianten sowie LTX-2.5 T2V/I2V/FLF2V und Wan Animate 2 Motion Transfer für Port 8188; Einrichtung: [docs/DUAL_GPU_WORKFLOWS.md](docs/DUAL_GPU_WORKFLOWS.md) |
...
| `Reference to Video` | 9 | MiniMax H3 FL2VA, Ref2VA, AutoLength und Complete-Song-Director; alle mit v4-Step-600-EMA-Turbo-LoRA |
...
Automatisch geprüft wurden alle 265 Workflows. Der `--against-head`-Modus prüft neue und geänderte Dateien vollständig, behandelt bereits eingecheckte Integrationen als unveränderte Baseline, normalisiert bei älteren Deltas ausschließlich die im 186-Dateien-Manifest erlaubten Prompt-/Pause-Transformationen und akzeptiert die H3-Turbo-Migration nur dann, wenn sie bytegenau aus dem deterministischen Integrator reproduzierbar ist:

- 265/265 gültige JSON-Dateien
- 248 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 324 Haupt- und Untergraphen rekursiv geprüft
- 9.886 Nodes und 4.183 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.991 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen und wechselseitiger Input-/Output-Zuordnung erfasst; die Workflow-12-Metrikpfade und alle acht Workflow-13-Ausgaben sind strukturell verbunden
- keine neuen doppelten IDs, fehlenden oder einseitigen Endpunkte, Note-Zuordnungs- oder Layoutfehler in den v0.9.1-Dateien
- die vier v0.9.0-Dual-GPU-Workflows wurden über den echten ComfyUI-Browsergraphen in API-Prompts expandiert und auf Windows-ROCm real ausgeführt: LTX-2.5 T2V, I2V und FLF2V jeweils mit 2 Sekunden bei 320×320 sowie Wan Animate 2 Motion Transfer mit 9 Frames bei 256×256. Alle vier Läufe endeten mit `execution_success` und speicherten nichtleere MP4-Dateien; Wan Animate 2 verwendet dabei den speichersicheren CPU-/INT8-Pose-Cache.
- die drei v0.9.1-MiniMax-Music-3-Workflows wurden ebenfalls über den echten Browsergraphen expandiert und mit den vollständigen lokalen Gewichten ausgeführt. Single-GPU und Dual-GPU verwendeten vier Sekunden Maximaldauer, alle 30 Euler-/Simple-Schritte und tiled DAV decode; beide speicherten endliche, nichtleere 44,1-kHz-Stereo-FLACs mit 7,988 Sekunden Laufzeit. Das Dual-GPU-Log bestätigte den Textstack auf `cuda:0` (R9700) und MODEL/DAV auf `cuda:1` (RX 9070 XT). Der Qwen-3.5-4B-Enhancer endete ebenfalls mit `execution_success` und lieferte exakt `Global Metadata`, `Vocal Details` und `Arrangement`.

# DaWasteh – konsolidierte ComfyUI-Workflows · v0.8.7
...
- **23 zusätzliche Dual-GPU-Workflows** für einen gemeinsamen ComfyUI-Server auf Port 8188: 21 kuratierte Modellfamilien-Varianten plus je ein offener MiniMax-H3-FL2VA- und Ref2VA-Graph; Diffusionsmodelle laufen auf der R9700 (`gpu:0`), CLIP/Textencoder und VAEs auf der RX 9070 XT (`gpu:1`)
...
- 241 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
- alle vorhandenen Workflows wurden mit deutlich größeren, nachvollziehbaren Abständen angeordnet
- **3.879 automatisch zugeordnete Parameter-Notes** erklären jeden Nicht-Pixaroma-/Nicht-Dokumentations-Node einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
- 9 lokale MiniMax-H3-Quellworkflows: FL2VA, Ref2VA, automatische Audio-/Videolänge sowie ein Complete-Song-Director, der lange Songs seriell in höchstens 15 Sekunden lange H3-Szenen zerlegt. v0.8.4 ergänzte Identity-Lock und unverändertes Originalaudio; v0.8.6 ergänzte Turbo-LoRA und Prompt-Enhancer; v0.8.7 liefert zusätzlich zwei offene Dual-GPU-Arbeitsgraphen für normale FL2VA- und Ref2VA-Anwendungen außerhalb des Musik-Directors. Einrichtung und Prüfnachweise: [docs/MINIMAX_H3_TURBO_AND_PROMPTS.md](docs/MINIMAX_H3_TURBO_AND_PROMPTS.md)
...
| Ordner | Anzahl | Zweck |
|---|---:|---|
| `Dual GPU - R9700 + RX 9070 XT` | 23 | 21 kuratierte Modellfamilien-Workflows plus offene MiniMax-H3-FL2VA-/Ref2VA-Varianten für Port 8188; Einrichtung: [docs/DUAL_GPU_WORKFLOWS.md](docs/DUAL_GPU_WORKFLOWS.md) |
...
Automatisch geprüft wurden alle 258 Workflows. Der `--against-head`-Modus prüft neue und geänderte Dateien vollständig, behandelt bereits eingecheckte Integrationen als unveränderte Baseline, normalisiert bei älteren Deltas ausschließlich die im 186-Dateien-Manifest erlaubten Prompt-/Pause-Transformationen und akzeptiert die H3-Turbo-Migration nur dann, wenn sie bytegenau aus dem deterministischen Integrator reproduzierbar ist:

- 258/258 gültige JSON-Dateien
- 241 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 309 Haupt- und Untergraphen rekursiv geprüft
- 9.249 Nodes und 3.879 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.359 Graph-Links einschließlich Slotgrenzen, Typkompatibilität und wechselseitiger Input-/Output-Zuordnung erfasst; die Workflow-12-Metrikpfade und alle acht Workflow-13-Ausgaben sind strukturell verbunden

# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.7
...
- **232 kuratierte Workflow-Dateien** in 31 Kategorien
...
- 215 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
...
Automatisch geprüft werden alle 232 Workflows. Vor dem Release rekonstruiert `--against-head` die beiden v0.9.7-Additionen deterministisch aus dem SHA-256-gepinnten offiziellen Pixal3D-Template und bewahrt gleichzeitig die v0.9.6-/v0.9.5-Migrationskette. Der Validator prüft zusätzlich die exakte Dateimenge sowie die früheren AutoSongwriter-/Wan-Animate-Upgrades:

- 232/232 gültige JSON-Dateien
- 215 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 285 Haupt- und Untergraphen rekursiv geprüft
- 10.399 Nodes und 4.733 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 7.179 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen, unabhängiger Low-Poly-LOD-Pfade, Pixal3D-PBR-Baking, AutoSongwriter-Genre/BPM/Tonart/Sprache/Dateiname-Verbindungen, Sekunden→Frame-Umrechnungen und wechselseitiger Input-/Output-Zuordnung erfasst

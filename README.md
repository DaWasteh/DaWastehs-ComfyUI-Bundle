# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.4
...
- **227 kuratierte Workflow-Dateien** in 30 Kategorien
...
- **Optionale GPU-Platzierung direkt in allen 227 Workflows**: Der frühere Ordner `Dual GPU - R9700 + RX 9070 XT` wurde aufgelöst. Jeder kanonische Workflow enthält jetzt genau einen zentralen `DaW Multi-GPU Device Control`-Node. 28 bereits kuratierte Profile behalten ihre bewährte Verteilung; die übrigen 199 starten vollständig auf der R9700 (`gpu:0`). Wo standardisierte ComfyUI-Objekte vom Typ MODEL, CLIP oder VAE vorhanden sind, steuern verbundene offizielle Selector-Nodes die Platzierung. Bei proprietären Custom-Modellobjekten bleibt der Control-Node bewusst passiv, statt eine wirkungslose Umplatzierung vorzutäuschen.
...
- **124 Pixaroma Prompt**-Eingaben sowie **9 Pause Text**-Freigaben in insgesamt 111 Workflows
- 210 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
...
- **4.558 automatisch zugeordnete Parameter-Notes** erklären jeden Nicht-Pixaroma-/Nicht-Dokumentations-Node einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
Automatisch geprüft werden alle 227 Workflows. Vor dem Release rekonstruiert `--against-head` die v0.9.4-Anpassungen deterministisch aus dem noch auf v0.9.3 stehenden `HEAD`; nach dem Release bewahrt `--baseline-ref v0.9.3` denselben Vergleich. Der Validator prüft zusätzlich die exakte Dateimenge, die v0.9.3-AutoSongwriter-Konsolidierung und den reproduzierbaren Wan-Animate-2-Umbau:

- 227/227 gültige JSON-Dateien
- 210 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 280 Haupt- und Untergraphen rekursiv geprüft
- 10.032 Nodes und 4.558 Dokumentations-/Parameter-Notes; die verbleibenden `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.934 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen, AutoSongwriter-Genre/BPM/Tonart/Sprache/Dateiname-Verbindungen, Sekunden→Frame-Umrechnungen und wechselseitiger Input-/Output-Zuordnung erfasst
- keine neuen doppelten IDs, fehlenden oder einseitigen Endpunkte, Note-Zuordnungs-, Gruppen- oder Layoutfehler in den v0.9.3-Dateien

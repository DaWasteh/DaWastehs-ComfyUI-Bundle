# DaWasteh – konsolidierte ComfyUI-Workflows · v0.9.6
...
- **230 kuratierte Workflow-Dateien** in 31 Kategorien
...
- **Optionale GPU-Platzierung direkt in allen 230 Workflows**: Der frühere Ordner `Dual GPU - R9700 + RX 9070 XT` wurde aufgelöst. Jeder kanonische Workflow enthält jetzt genau einen zentralen `DaW Multi-GPU Device Control`-Node. 28 bereits kuratierte Profile behalten ihre bewährte Verteilung; die übrigen 202 starten vollständig auf der R9700 (`gpu:0`). Wo standardisierte ComfyUI-Objekte vom Typ MODEL, CLIP oder VAE vorhanden sind, steuern verbundene offizielle Selector-Nodes die Platzierung. Bei proprietären Custom-Modellobjekten bleibt der Control-Node bewusst passiv, statt eine wirkungslose Umplatzierung vorzutäuschen.
...
- 213 Nicht-Live-Workflows enthalten exakt einen `PixaromaRunTimer`; die 17 Live-Avatar-Workflows bleiben auf ausdrücklichen Nutzerwunsch timerfrei
...
- **4.613 Dokumentations-/Parameter-Notes** erklären die Workflow-Nodes einschließlich aktueller Werte, Schalterwirkung, Ein-/Ausgänge und höher-/niedriger-Auswirkung
...
- 2 neue Godot-/Game-Development-Startworkflows: FLUX.2-Klein-PS1-Texturkonzept sowie Hunyuan3D-Core-Decimation zu einem statischen Low-Poly-GLB
...
| `Game Development` | 2 | FLUX.2-Klein-PS1-Texturkonzept sowie Hunyuan3D-LOD0–3 mit unabhängiger Silhouettenstärkung, UVs und Normalen für Godot |
...
Automatisch geprüft werden alle 230 Workflows. Vor dem Release rekonstruiert `--against-head` die v0.9.6-LOD-Änderung deterministisch aus dem noch auf v0.9.5 stehenden `HEAD`; nach dem Release bewahrt `--baseline-ref v0.9.5` denselben Vergleich. Die ältere v0.9.5-Ableitung aus v0.9.4 bleibt Teil derselben Migrationskette. Der Validator prüft zusätzlich die exakte Dateimenge sowie die früheren AutoSongwriter-/Wan-Animate-Upgrades:

- 230/230 gültige JSON-Dateien
- 213 Nicht-Live-Workflows mit genau einem `PixaromaRunTimer`; 17/17 Live-Avatar-Workflows ohne Run-Timer
- 283 Haupt- und Untergraphen rekursiv geprüft
- 10.151 Nodes und 4.613 Dokumentations-/Parameter-Notes; die vorhandenen `PixaromaPrompt`-/`PixaromaPauseText`-Integrationen bleiben erhalten
- 6.993 Graph-Links einschließlich Slotgrenzen, Typkompatibilität, zentraler Device-Control-Verbindungen, unabhängiger Low-Poly-LOD-Pfade, AutoSongwriter-Genre/BPM/Tonart/Sprache/Dateiname-Verbindungen, Sekunden→Frame-Umrechnungen und wechselseitiger Input-/Output-Zuordnung erfasst

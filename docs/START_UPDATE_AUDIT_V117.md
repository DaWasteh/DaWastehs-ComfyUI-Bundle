...
- ComfyUI **0.35.0**, `6338e4bd428247a4a8843496aa98fb7f2a9d3632` vom 10. September. `git ls-remote https://github.com/comfy-org/ComfyUI.git HEAD` ergab denselben Commit: zum Prüfzeitpunkt aktuellster Upstream-Core.
...
**Kein neuer Geschwindigkeitsgewinn behauptet.** Dies ist ein Kompatibilitäts-/Sicherheitsrelease mit unveränderten Performance-Defaults. Die alten Messungen belegen eine konservative Ausgangsbasis, nicht das absolute Optimum des neuen Stacks. Während dieser Prüfung testete eine andere Sitzung HIP/Vulkan-LLMs; deshalb keine konkurrierenden Modell-, Trainings- oder RAM-Stresstests. Erst nach deren Freigabe lief ein isolierter Serverstart ohne Inferenz. Der VRAM-Guard berücksichtigt freien Speicher beim Initialisieren, nicht spätere Allokationen anderer Programme.
...
2. **Erste Launcher-Übernahme ohne Datenverlust.** Historische, unveränderte Git-Inhalte werden auch bei Windows-Zeilenenden/BOM erkannt. Persönliche oder unbekannte Änderungen werden nicht ungefragt ersetzt. Anschließend erfolgt immer eine binäre Commit-Blob-Kopie. Erhaltene Dateien tragen im Manifest zusätzlich ihren tatsächlichen `preserved_sha256`, werden gegen genau diesen Hash validiert und ausdrücklich als **nicht synchronisiert** gemeldet. Der erwartete Quell-Hash bleibt separat erhalten, damit der nächste Lauf die persönliche Änderung ebenfalls schützt. Eine während des Updates erneut veränderte Datei löst einen Fehler aus. `-Force` bleibt eine bewusste, globale Überschreiboption mit Backup.
...
- Isolierter ComfyUI-0.35.0-Start auf Port 8190 mit dem bestehenden Bench-Controller/Guard-Profil, Manager aus, eigene DB/Output/Temp: nach 20,4 s bereit; `/system_stats` und `/rdna4/info` bestätigen `attention_pytorch`, `NORMAL_VRAM`, 0 Offload-Streams, Pinned Memory aus, `aimdo=False`, beide VRAM-Guards aktiv. Keine Prompts oder Modelle geladen. Bekannte optionale KJNodes-Triton-VAE-Imports bleiben unter Windows-ROCm ohne Triton nicht verfügbar. Der Testprozessbaum wurde danach beendet; `CTRL_BREAK` aus dem nichtinteraktiven Harness scheiterte mit WinError 87, daher nutzte der Bench-Controller seinen gezielten Taskkill-Fallback. Ein erfolgreicher interaktiver Ctrl+C-Stop wird damit nicht behauptet; Produktions-Launcher-Supervisor unverändert.
- Gesamtsuite: `L:/ComfyUI/.venv/Scripts/python.exe -B -m unittest discover -s tests`: **297 Tests, 296 bestanden, 1 bestehender Skip**, keine Fehler.
